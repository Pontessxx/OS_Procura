...
class SimpleApp(ctk.CTk):
    def __init__(self):
        ...
        self.new_window_button = ctk.CTkButton(self.frame, text="Cadastrar nova OS", command=self.abrir_nova_janela)
        ...
        self.style_treeview.configure(
            "Treeview.Heading",
            background=self.my_dic['Frame_Preto'],
            foreground=self.my_dic['font'],
            borderwidth=1,
            relief='solid',
            font=('Arial', 10)
        )
        ...
        self.style_treeview.configure(
            ...
            background=self.my_dic['Frame_Preto'],
            foreground=self.my_dic['font'],
            fieldbackground=self.my_dic['Frame_Preto'],
            rowheight=25,
            borderwidth=1,
            relief='solid'
        )
        self.style_treeview.map(
            "Treeview",
            background=[('selected', self.my_dic['Vermelho_Brad'])],
    ...
    def filtrar_acesso(self, ano, mes, tipo_os, site, categoria_primaria, categoria_secundaria):
        try:
            ...
            self.imprimir_ultimas_colunas(resultados, False)
            return resultados
        except pyodbc.Error as e:
            messagebox.showerror("Erro", f"Erro ao filtrar dados: {e}")
            return None

    def imprimir_ultimas_colunas(self, resultados, segunda_busca=False):
        ...
        if resultados:
            for row in resultados:
                if segunda_busca:
                    ...
                    code_os = row[5]
                    path_erro = row[-1]
                    self.encontrar_os_path(path, code_os, path_erro, True)
                else:
                    path = row[-3]
                    code_os = row[4]
                    path_erro = row[-1]
                    self.encontrar_os_path(path, code_os, path_erro, False)
                print(f'\n [PATH]: {path}\n [CODIGO OS]: {code_os}\n [PATH_erro]: {path_erro}\n')
        else:
    ...
    def encontrar_os_path(self, path, code_os, path_erro, segunda_busca_os):
        ...
        if not segunda_busca_os:
            os_ = code_os + self.input_value.get()
            os_space = code_os + ' ' + self.input_value.get()
        else:
            input_user = self.input_value.get().lower()
            code_remove = code_os.lower()
            os_ = input_user.replace(code_remove, '')
            os_ = code_os + os_
            os_space = code_os + ' ' + os_
        found = False
        for item in lista_path:
            if item.startswith(os_) or item.startswith(os_space):
                ...
                break
        if not found:
            print(f'item correspode a {os_} ou {os_space} não encontrado')
    ...
    def query_os_referencia_teste(self, input_str):
        try:
            ...
            if resultados:
                for row in resultados:
                    self.tipo_combobox.set(row[-2])
                    self.site_combobox.set(row[2])
                    self.categoria_primaria.set(row[3])
                    if row[4] is not None:
                        self.categoria_secundaria.set(row[4])
                    else:
                        self.categoria_secundaria.set('')
                    # You can add logic here to handle the retrieved rows.
                    # Example: self.tabela.insert("", "end", values=row)
                    print(row)
                self.imprimir_ultimas_colunas(resultados, True)
            else:
    ...
    def cadastrar_os(self):
        ...
        try:
            ...
            if resultado_existente:
                ...
                insert_query = """
                    INSERT INTO Procura (Código, [CODIGO OS], OS, SITES, ANOS, MES, CATEGORIA_PRIMARIA, CATEGORIA_SECUNDARIA, PATH, PATH_erro)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """
    ...
    def abrir_nova_janela(self):
        ...
        self.categoria_primaria_nova = ctk.CTkComboBox(nova_janela, values=self.categoria_primaria.cget("values"), state='readonly')
        ...
        self.categoria_secundaria_nova = ctk.CTkComboBox(nova_janela, values=self.categoria_secundaria.cget("values"), state='readonly')
        ...
        self.path_entry = ctk.CTkEntry(nova_janela, placeholder_text="C:\\")  # Ajuste do width
        ...
        self.codigo_os_entry = ctk.CTkEntry(nova_janela, placeholder_text="EEc")
